...
// codexExecItem mirrors the "item" object inside item.completed events
// emitted by `codex exec --json`.
...
// codexExecEvent is a single NDJSON line produced by `codex exec --json`.
...
func (r *CodexRunner) RunNew(ctx context.Context, prompt string) (*RunResult, error) {
	args := []string{"exec"}
...
func (r *CodexRunner) RunResume(ctx context.Context, sessionID, prompt string) (*RunResult, error) {
...
func (r *CodexRunner) run(ctx context.Context, args []string) (*RunResult, error) {
	...
	for scanner.Scan() {
		...
		case ev.Type == "thread.started" && ev.ThreadID != "":
			result.EngineSessionID = ev.ThreadID
